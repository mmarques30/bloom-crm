      ...
      <div className="p-6">
        <Tabs defaultValue="ativas">
          <TabsList className="pill mb-4 bg-muted">
            <Can permission="tab.campanhas.ativas">
              <TabsTrigger value="ativas" className="pill">Ativas</TabsTrigger>
            </Can>
            <Can permission="tab.campanhas.rascunhos">
              <TabsTrigger value="rascunhos" className="pill">Rascunhos</TabsTrigger>
            </Can>
            <Can permission="tab.campanhas.relatorios">
              <TabsTrigger value="relatorios" className="pill">Relatórios</TabsTrigger>
            </Can>
          </TabsList>
          ...
          <TabsContent value="rascunhos">
            <div className="grid gap-4 md:grid-cols-2">
              {campaigns.filter((c) => c.status === "Rascunho").map((c) => (
                <CampaignCard key={c.name} c={c} />
              ))}
            </div>
          </TabsContent>
          <TabsContent value="relatorios">
            <div className="rounded-2xl border border-border bg-card p-8 text-sm text-muted-foreground">
              Relatórios consolidados por campanha (em breve).
            </div>
          </TabsContent>
        </Tabs>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <span
          className={
            ...
            (c.status === "Ativa" ? "bg-primary/10 text-primary" : "bg-secondary text-secondary-foreground")
          }
        >
          {c.status}
        </span>
